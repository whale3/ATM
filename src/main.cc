#include<iostream>
#include<string.h>
#include"account.hpp"


struct Info
{
    std::string Name;//用户名
    std::string Passwd;//密码
    double Balance;//余额
};
class User
{
public:
    void Menu1();//菜单1是登陆注册页面
    void Menu2(const std::string& username);//菜单2是操作界面
    void Login();//登陆
    void Sign_up();//注册
    void Query(const std::string& username);//查询
    void Deposit(const std::string& username);//存钱
    void Withdraw(const std::string& username);//取钱
    void Transfer(const std::string& username);//转账


};
void User::Menu1()
{
    std::string choose;
    //char choose;
    while(true)
    {
        system("clear");
        std::cout<<std::endl;
        std::cout<<std::endl;
        std::cout<<"        康氏银行自动存取款机"<<std::endl;
        std::cout<<std::endl;
        std::cout<<"    1、登陆"<<std::endl;
        std::cout<<std::endl;
        std::cout<<"    2、注册"<<std::endl;
        std::cout<<std::endl;
        std::cout<<"    3、退出"<<std::endl;
        getline(std::cin,choose);
        //std::cin>>choose;
        //}while(choose != 3 || choose != 1 || choose !=2);

    if(choose=="1")
    {
        Login();
    }
    else if(choose=="2")
    {
        Sign_up();
    }
    else if(choose=="3")
    {
       break; 
    }
    }
   //else return 1;
     //  break;
   // break;
}
void User::Menu2(const std::string& username)
{
    std::string choose;
    while (true ) {
        system("clear");
        std::cout<<std::endl;
        std::cout<<std::endl;
        std::cout<<"        康氏银行欢迎您！"<<username<<std::endl;
        std::cout<<std::endl;
        std::cout<<"    1、查询"<<std::endl;
        std::cout<<std::endl;
        std::cout<<"    2、取钱"<<std::endl;
        std::cout<<std::endl;
        std::cout<<"    3、存钱"<<std::endl;
        std::cout<<std::endl;
        std::cout<<"    4、转账"<<std::endl;
        std::cout<<std::endl;
        std::cout<<"    5、退出"<<std::endl;
        getline(std::cin,choose);
    if(choose=="1")
    {
        Query(username);
    }
    else if(choose=="2")
    {
        Withdraw(username);
    }
    else if(choose=="3")
    {
        Deposit(username);
    }
    else if(choose=="4")
    {
        Transfer(username);
    }
    else if(choose=="5")
    {
        return;
        //break;
    }
    else 
    {
        std::cout<<"请输入1-5的数字进行选择"<<std::endl;
    }
    }
}
void User::Login()
{   
    AccountDAO a;
    Info info;
    int i;
    for(i=0; i<3;i++)
    { 
      try{
          system("clear");
          std::cout<<"请输入用户名： ";
          getline(std::cin,info.Name);
          std::cout<<"请输入密码： ";
          system("stty -echo");
          getline(std::cin,info.Passwd);
          system("stty echo");
          a.auth(info.Name,info.Passwd);
          break;
      }catch(const std::runtime_error& e) {
        std::cout<<"用户名或者密码错误，还有"<<2-i<<"次机会!"<< std::endl<<"继续输入按'y',返回上一级页面按'q'! "<<e.what()<<std::endl;
        std::string c;
        getline(std::cin,c);
        if(c=="y"|| c=="Y")
        {   
            continue;//以下不执行了，从循环处开始执行
        }
        if(c=="q"||c=="Q")
        {
             //Menu1();
             break;
        }
        else
            break;
        throw e;
      }
   }
   
   if (i == 3)
       std::cout<<"请去柜台核实账号"<<std::endl;
   else
       Menu2(info.Name);
}
void User::Sign_up()
{   
    try{
    AccountDAO a;
    Info info;
    system("clear");
    std::cout<<"请输入用户名： ";
    getline(std::cin,info.Name);
    std::cout<<"请输入密码： ";
    system("stty -echo");
    getline(std::cin,info.Passwd);
    system("stty echo");
    //std::cout<<std::endl<<"请再输入密码： ";
    //system("stty -echo");
    //getline(std::cin,info.Passwd);
    //system("stty echo");
    a.insert(info.Name,info.Passwd,0.0);
    std::cout<<std::endl<<"注册成功！"<<std::endl<<"跳转至登陆界面，确认按'y',按任意键退出!"<<std::endl;
    std::string c;
    getline(std::cin,c);

    if(c=="y"||c=="Y")
    {
        Menu1();

    }
    }catch(const std::runtime_error& e){
        std::cout<<e.what()<<std::endl;
            throw e;
        }
    
}
void User::Query(const std::string& username)
{
    AccountDAO a;
    Info info;
    int sum;
    system("clear");
    sum = a.balance(username);
    std::cout<<"您的用户还剩余额： "<<sum<<std::endl;
    std::cout<<"按任意键返回主菜单,退出登录请按'q'"<<std::endl;
    std::string c;
    getline(std::cin,c);
    if(c=="q"||c=="Q")
    {
        Menu1();
    }
    else 
    {
        return ;
    }
}
void User::Deposit(const std::string& username)
{
    AccountDAO a;
    Info info;
    int money;
    std::string money_s;
    std::string c;
    system("clear");
    std::cout<<"您存入的金额为： "<<std::endl;
    getline(std::cin,money_s);
    money=std::atoi(money_s.c_str());
    std::cout<<"您要存入的金额为："<<money<<std::endl;
    std::cout<<"确认请按'y',按任意键取消并返回上一个界面"<<std::endl;
    getline(std::cin,c);
    if(c=="y"||c=="Y")
    {       
        a.deposit(username,money);
        std::cout<<"存入成功！"<<std::endl;
        std::cout<<"继续存钱请按'c',按任意键返回上一个界面"<<std::endl;
        getline(std::cin,c);
        if(c=="c"||c=="C")
        {
           Deposit(username);
        }
        else 
        {
            return ;
        }
        

    }
    else
    {
        return ;
    }
}
void User::Withdraw(const std::string& username)
{
         AccountDAO a;
         Info info;
         int money;
         std::string money_s;
         std::string c;
         while(true) {
    try{
             system("clear");
             std::cout<<"您取出的金额为： "<<std::endl;
             getline(std::cin,money_s);
             money=std::atoi(money_s.c_str());
             std::cout<<"您要取出的金额为："<<money<<std::endl;
             std::cout<<"确认请按'y',按任意键取消并返回上一个界面"<<std::endl;
             getline(std::cin,c);
             if(c=="y"||c=="Y") {       
                 a.withdraw(username,money);
                 std::cout<<"取出成功！"<<std::endl;
                 std::cout<<"继续取钱请按'c',按任意键返回上一个界面"<<std::endl;
                 getline(std::cin,c);
                 if(c=="c"||c=="C")
                 {
                        Withdraw(username);
                 }
                 else 
                 {
                     return ;
                 }
             }
             else 
             {
                 return ;
             }
    }catch(const std::runtime_error& e){
        std::cout<<"您要取出的金额超过您的余额，请按任意键之后再进行重新输入取出金额!"<<std::endl;
        //std::cout<<e.what()<<std::endl;
        getline(std::cin,c);
        continue;
        }
       }

}
void User::Transfer(const std::string& username)
{
    AccountDAO a;
    Info info;
    int money;
    std::string money_s;
    std::string c;
    std::string name_to;
    while(true)
    {
        try
        {
             system("clear");
             std::cout<<"您要转入的账号是： "<<std::endl;
             getline(std::cin,name_to);
             std::cout<<"您要转出的金额是： "<<std::endl;
             getline(std::cin,money_s);
             money=std::atoi(money_s.c_str());//将string 转换成int型
         
             std::cout<<"您要转入的账号是："<<name_to<<std::endl<<"您要转出的金额是："<<money<<std::endl<<"确认请按'y',按任意键取消并返回上一个页面！"<<std::endl;
             getline(std::cin,c);
             if(c=="y"||c=="Y")
             {
                 a.transfer(username,name_to,money);
         
             }
             else {
                  return ;
             }
             std::cout<<"转出成功！"<<std::endl;
             std::cout<<"继续转钱请按'c',按任意键取消并返回上一个页面！"<<std::endl;
             getline(std::cin,c);
             if(c=="c"||c=="C")
             {
                Transfer(username);
             }
             else
             {
                return;
             }
         }catch(const std::runtime_error& e){
        std::cout<<"您要转出的账号错误或金额超出您的余额，请按任意键之后再进行重新输入！："<<std::endl;
        //std::cout<<e.what()<<std::endl;
        getline(std::cin,c);
        continue;
        }
     }
             
    
    
}
int main(int argc,char* argv[])
{   
    Dbpath = argv[1];

    User user;
    user.Menu1();
    return 0;
}
